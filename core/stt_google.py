...
class GoogleCloudSTT(STTEngine):
    ...
    def __init__(
        ...
        logger.info(
            "GoogleCloudSTT configured (model={}, lang={}, free 60 min/month)",
            model,
            self._language,
        )
    ...
    async def get_transcripts(self) -> AsyncGenerator[TranscriptEvent, None]:
        ...
        # Send raw PCM bytes (not WAV-wrapped) with LINEAR16 encoding
        ...
        try:
            ...
            for result in results:
                ...
                if self._enable_word_time_offsets:
                    for w in best.get("words", []):
                        ...
                        words.append(
                            WordInfo(
                                word=w.get("word", ""),
                                start_ms=_parse_duration_ms(start_time),
                                end_ms=_parse_duration_ms(end_time),
                ...
                logger.info(
                    "Google Cloud transcription ({:.1f}s audio, model={}): '{}'",
                    duration_ms / 1000,
                    self._model,
                    text[:100],
                )
